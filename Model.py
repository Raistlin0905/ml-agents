...
class Model(AbstractBaseClass, Generic[Features, Label]):
    ...
    def train(self, training_data: list[tuple[Features, Label]]) -> None:
        ...
        """
        pass
    def test(self, validation_data: list[tuple[Features, Label]],folds: int, w_0:float, w_1: float) -> tuple[float, float, float]:
        """
        ...
        """
        cv = KCrossValidation(validation_data=validation_data, folds=folds, model=self, loss_function=KCrossValidation.SRLossStartegy)
        a, t = cv.run()
...
class KCrossValidation(Generic[Features, Label]):
    def __init__(self, validation_data: list[tuple[Features, Label]], folds: int, model: Model[Features, Label], loss_function: Callable[[Label, Label], float]):
    ...
    def run(self) -> tuple[float, float]:
        ...
        for i in range(len(partitions)):
            ...
            startTime = time.time()
            testPredictionLabels = self.model.predict(testFeatures) #list of predictions by the model
            endTime = time.time()

            locPartitionInferenceTime = (endTime - startTime) #!!FOR THE WHOLE PARTITION (not individual data points)
